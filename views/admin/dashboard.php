...
<?php if (count($posts) <= 0) : ?>
    <div class="flex flex-col items-center justify-center w-full gap-2 px-8 py-16 mt-4 text-2xl font-semibold text-center text-gray-500">
        ...
        <a href="<?= $router->route("post.create") ?>" class="text-base underline hover:text-blue-500">Cadastrar o primeiro post</a>
    </div>
<?php else : ?>
    <div class="flex flex-col md:flex-row items-start md:items-center md:justify-between mb-4 w-full gap-2">
        <h1 class="text-2xl font-bold">Posts</h1>
        <a href="<?= $router->route("post.create") ?>" class="inline-flex items-center bg-gray-100 border-0 py-1 px-3 focus:outline-none hover:bg-gray-200 rounded text-base">Novo
        ...
        <table class="table-auto w-full text-left whitespace-no-wrap">
            <thead>
                <tr>
                    ...
                    <th class="px-4 py-3 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100">Postado em</th>
            ...
            <tbody>
                <?php foreach ($posts as $post) : ?>
                    <tr>
                        <td class="px-4 py-3 align-middle"><?= $post->title ?></td>
                        <td class="px-4 py-3 align-middle"><?= date("d/m/Y H:i", strtotime($post->created_at)) ?></td>
                        <td class="w-10 text-center align-middle">
                            <div class="flex items-center gap-2">
                                <a href="<?= $router->route("post.edit", ["id" => $post->id]) ?>" class="inline-flex items-center bg-green-100 border-0 py-2 px-3 focus:outline-none hover:bg-green-200 rounded text-base">
                                ...
                                <form action="<?= $router->route("post.delete", ["id" => $post->id]) ?>" method="post">
